...
提示：
建议使用“pushplus推送加”接收检测文章，微信公众号关注“pushplus推送加”，点击pushplus进入到官网首页注册并激活消息，注册后获取您token口令填写到下方( or "口令" )。当然您也可以使用企业微信和wxpusher接收消息。
每天前两篇检测文章不过，有黑号的风险，导致阅读无效，如不能及时接收检测文章，或不能及时阅读检测文章，建议手动运行脚本，运行前去微信手动阅读三篇文章，每篇阅读6秒以上。
每天180个任务不建议跑满，细水长流，如出现阅读更新中，说明检测文章阅读无效，建议4小时后再操作。平时在订阅号多读文章，多点赞评论的方式养号，增加阅读成功率。
