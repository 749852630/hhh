...
class Mttb:
        ...
        while try_count > 0:
            try:
                ...
                params = {
                    ...
                    'csecplatform': '4',
                    'csecversion': '2.3.1',
                    'mtgsig': '{"a1":"1.1","a2":1703739215722,"a3":"1703731650027OUCUAQQ52588f39db0d1b3f1e494e6fb36dd88b3171","a5":"AtaulsW36TB2AB+UiaNu2W==","a6":"hs1.4gUG48Qv2uNxUU1wCU+Cm0w59FyC5C1Gicd5H9C440aWjBVuLpPGQI8OT5AxCwHn3pqPtbAzEpRF+Ws3TpveWMUN47UF+JRVaCm7Gy1XiN5o=","x0":4,"d1":"02674551e52506c0a4824003e7424383"}',
                }
                ...
                if isdl:
                    r = requests.post(url, params=params, headers=h, json=data, timeout=10, verify=False, proxies=proxy)
                else:
                    r = requests.post(url, params=params, headers=h, json=data, timeout=10, verify=False)
                rj = r.json()
        ...
        while try_count > 0:
            try:
                headers = {
                    ...
                    # 'User-Agent': 'Mozilla/5.0 (Linux; Android 11; Redmi Note 8 Pro Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.141 Mobile Safari/537.36 TitansX/13.0.3 KNB/1.2.0 android/11 mt/com.sankuai.meituan/12.17.203 App/10120/12.17.203 MeituanGroup/12.17.203',
                    'User-Agent': self.ua,
                    'Content-Type': 'application/json',
                    'Accept': '*/*',
                    'Origin': 'https://mgc.meituan.com',
                    ...
                    'Sec-Fetch-Mode': 'cors',
                    'Sec-Fetch-Dest': 'empty',
                    # 'Referer': 'https://mgc.meituan.com/h5/gamehallv2.html?inner_source=10102_ch290&f=android&token=AgH9J7VZB3aF4GBQ6HTnzqO5dVtLgCvExihwnth6NLPLinuDt9m4SefzetQTNmlrURkJTXn-8UZENwAAAAAPHQAAyME9_mVkq2GSWNhJ88vaSfYy-b7Fxzz0xNUNyWz3XvWij_ejNuqQy7XB58dhu2F1&userid=4208094795&utm_source=xiaomi&utm_medium=android&utm_term=1200170203&version_name=12.17.203&utm_content=a5345ef4d660489e9c43a917f634d7cea170077938013221397&utm_campaign=AgroupBgroupC0D500E0Gmine&ci=351&msid=a5345ef4d660489e9c43a917f634d7cea1700779380132213971703750822123&uuid=00000000000006A93E32064A14317AC43DAB028157531A170368537562384040&p_appid=10',
                    'Accept-Encoding': 'gzip, deflate',
                    'Accept-Language': 'zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7',
                }
                sjc = int(time.time() * 1000.0000001)
    ...
    def main(self):
        ...
        if self.login():
            self.act()
            if self.startcxtb():
                self.ids = [15610, 15224, 15293, 15604, 15813, 15177, 15601, 15605, 15326, 15837, 15400, 15602, 15458,
                            15457, 15603, 15606, 15324, 15810, 15937, 15607, 15618, 16656, 15608, 16498, 15609, 15302,
                            717, 15282, 15287, 323, 324, 325, 326, 330, 331, 332, 333, 386, 393, 394, 395, 397, 507,
                            511, 525, 526, 527, 528, 529, 672, 768,
                            509, 510]
                self.coin_login()
                self.runq_jrtb()
                if self.coins > jrtb_nums:
                    ...
                    self.startmsg += f'🚀即将运行游戏中心任务和团币中心任务\n'
                    print(self.startmsg)
                    # self.run_tbrw()
                    if self.stoprw:
                        self.endmsg += f'🆔账号{self.num}-{self.name}({self.usid}) ⛔️设备风险较高\n'
                        pass
                    else:
                        self.game_gift()
                        self.run_game()
                ...
                if self.endcxtb():
                    if self.tj_bchd():
                        ...
                        self.endmsg += f'🆔账号{self.num}-{self.name}({self.usid})⛔️获取今日团币失败\n'
                        print(self.endmsg)
                else:
                    self.endmsg += f'🆔账号{self.num}-{self.name}({self.usid})⛔️获取运行后团币失败\n'
                    print(self.endmsg)
            else:
